...
(*
external init_timerFunc : (int -> unit) -> unit = "init_timerfunc"
*)
let timer_hashtbl = Hashtbl.create 16
let real_call_back i = (Hashtbl.find timer_hashtbl i) ()
(*
let _ = init_timerFunc real_call_back
*)
let _ = Callback.register "Alleg timer callback" real_call_back;;

external _install_param_int: ms:int -> i:int -> unit = "ml_install_param_int"

let timer_count = ref 0

let install_param_int ~ms ~(cb:(param:'a -> unit)) ~param =
  let i = !timer_count in
  incr timer_count;
  Hashtbl.add timer_hashtbl i (fun () -> cb param);
  _install_param_int ~ms ~i;
  (i)
;;

external _remove_param_int: i:cb_id -> unit = "ml_remove_param_int"
let remove_param_int ~id =
  try
    Hashtbl.remove timer_hashtbl id;
    _remove_param_int ~i:id;
  with Not_found ->
...
let do_circle ~bmp ~x ~y ~radius ~d ~proc =
  Callback.register "Alleg callback do_circle" proc;
...
external do_ellipse: bmp:bitmap -> x:int -> y:int -> rx:int -> ry:int -> d:int -> unit
    = "ml_do_ellipse_bytecode"
      "ml_do_ellipse_native"

let do_ellipse ~bmp ~x ~y ~rx ~ry ~d ~proc =
  Callback.register "Alleg callback do_ellipse" proc;
...
let textprintf_ex ~bmp ~f ~x ~y ~color ~bg fmt =
  Printf.ksprintf (fun str ->
      textout_ex ~bmp ~f ~str ~x ~y ~color ~bg;
  ) fmt
;;
*)
...
let textprintf_centre_ex bmp f x y color bg fmt =
  Printf.ksprintf (fun str ->
      textout_centre_ex ~bmp ~f ~str ~x ~y ~color ~bg;
  ) fmt
;;
...
let textprintf_justify_ex bmp f x1 x2 y diff color bg fmt =
  Printf.ksprintf (fun str ->
      textout_justify_ex ~bmp ~f ~str ~x1 ~x2 ~y ~diff ~color ~bg;
  ) fmt
;;
...
let textprintf_right_ex bmp f x y color bg fmt =
  Printf.ksprintf (fun str ->
      textout_right_ex ~bmp ~f ~str ~x ~y ~color ~bg;
  ) fmt
;;
...
external drawing_mode_1: draw_mode:_draw_mode -> unit = "ml_drawing_mode_1"
external drawing_mode_pattern: draw_mode:_draw_mode -> pattern:bitmap -> x_anchor:int -> y_anchor:int -> unit = "ml_drawing_mode_pattern"
...
let drawing_mode ~draw_mode =
  match draw_mode with
  | DRAW_MODE_SOLID -> drawing_mode_1 DM_SOLID
  | DRAW_MODE_XOR -> drawing_mode_1 DM_XOR
  | DRAW_MODE_COPY_PATTERN (pattern, x, y) -> drawing_mode_pattern DM_COPY_PATTERN pattern x y
  | DRAW_MODE_SOLID_PATTERN (pattern, x, y) -> drawing_mode_pattern DM_SOLID_PATTERN pattern x y
  | DRAW_MODE_MASKED_PATTERN (pattern, x, y) -> drawing_mode_pattern DM_MASKED_PATTERN pattern x y
